...
const GameMenu: React.FC<GameMenuProps> = ({ onGameSelect, onBackToHome }) => {
  const getIcon = (iconName: string) => {
    switch (iconName) {
      ...
      case 'rotate-ccw': return <RotateCcw className="w-8 h-8 text-white" />;
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {games.map((game) => (
            <div
              ...
              className="group bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10 hover:border-white/20 cursor-pointer transition-all duration-300 hover:transform hover:scale-105"
            >
              {/* Icon */}
              <div className={`w-16 h-16 bg-gradient-to-r ${game.gradient} rounded-xl flex items-center justify-center mb-4 group-hover:shadow-lg transition-shadow`}>
                {getIcon(game.icon)}
              </div>
              ...
              <div className="pt-4 border-t border-white/10">
                <div className="text-center text-sm text-slate-400 group-hover:text-white transition-colors">
                  Click to play →
                </div>
              </div>
            </div>
